          ...
          <table class="table table-bordered table-striped">
            ...
            <tbody>
              <tr v-for="user in users" :key="user.id">
                ...
                <td>
                  <select v-model="user.selectedRole" @change="updateUserRole(user.id, user.selectedRole)" class="form-select">
  ...
  export default {
    ...
    methods: {
      ...
      async updateUserRole(userId, roleId) {
        try {
          await axios.post('http://localhost:8000/api/updateUserRole', {
            user_id: userId,
            role_id: roleId
          })
        } catch (error) {
